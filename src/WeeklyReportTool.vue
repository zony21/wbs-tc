...
function projectNameForTask(state: AppState, task: WbsTask) {
  ...
  const project = state.projects.find(
    (candidate) => candidate.id === section?.projectId,
  );
...
function normalizeMonday() {
  ...
  selectedMonday.value = toDateKey(
    mondayOf(parseDateKey(selectedMonday.value)),
  );
...
function buildWeeklyReport(state: AppState) {
  const weekStart = mondayOf(
    parseDateKey(selectedMonday.value || currentMondayKey()),
  );
  ...
  const reports = state.reports
    ...
    .filter(
      (report) => report.date >= weekStartKey && report.date <= weekEndKey,
    );

  const overtime = reports.reduce(
    (sum, report) => sum + (Number(report.overtimeHours) || 0),
    0,
  );
  ...
  for (const report of reports) {
    for (const entry of report.entries) {
      taskHours.set(
        entry.taskId,
        (taskHours.get(entry.taskId) || 0) + (Number(entry.hours) || 0),
      );
    }
  }
  ...
  const achievements = [...taskHours.entries()]
    ...
    .filter((item): item is { task: WbsTask; hours: number } =>
      Boolean(item.task),
    )
  ...
  const plans = state.tasks
    .filter(
      (task) =>
        task.progress < 100 &&
        task.startDate <= nextEndKey &&
        task.dueDate >= nextStartKey,
    )
    .sort(
...
onMounted(() =>
  window.addEventListener("wbs-open-weekly-report", generateReport),
);
onBeforeUnmount(() =>
  window.removeEventListener("wbs-open-weekly-report", generateReport),
);
</script>
    ...
    >
      <header>
        <div>
          ...
          <span
            >対象週の日報を集計し、案件名・タスク名・タスク進捗を反映します。</span
          >
        </div>
        <button type="button" aria-label="閉じる" @click="open = false">
          ×
        </button>
      </header>

      <div class="weekly-selector">
        <button type="button" aria-label="前週" @click="shiftWeek(-1)">
          ‹
        </button>
        <label>
          <span>対象週（月曜日）</span>
          <input
            v-model="selectedMonday"
            type="date"
            @change="normalizeMonday"
          />
        </label>
      ...
      <template v-else>
        ...
        <template v-else>
          ...
          <p class="weekly-help">
            対象週の実績・残業時間は正式登録済みの日報から集計します。仮保存の日報は週報に含めません。%はWBSタスクの進捗率です。
          </p>
          <textarea
            id="weekly-report-output"
            v-model="outputText"
            rows="14"
          ></textarea>
        </template>
      ...
      <footer>
        <span class="weekly-copy-status" aria-live="polite">{{
          copyStatus
        }}</span>
        <div>
          <button type="button" class="weekly-secondary" @click="open = false">
            日報確認へ戻る
          </button>
          <button
            v-if="!loading && !errorMessage"
            type="button"
            class="weekly-primary"
            @click="copyOutput"
          >
            テキストをコピー
          </button>
...
.weekly-modal {
  ...
  border-radius: 18px;
  background: #fff;
  box-shadow: 0 24px 70px rgba(15, 31, 49, 0.28);
}
.weekly-modal header,
.weekly-modal footer {
  display: flex;
